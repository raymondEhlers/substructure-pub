...
void RunUnfolding(const std::string groomingMethod)
{
  ...
  // Setup
  // The iteration to load for the PseudoData and PseudoTrue.
  int nIterToLoad = 3;
  // Unfolding type
  ...
  std::string tagInput = "broadTrueBins";
  // And an optional tag at the end...
  std::string tag = "closureIter";
  ...
  if (tagInput != "") {
      inputFilename += "_" + tagInput;
  }
  if (tag != "") {
    outputFilename += "_" + tag;
  }
  outputFilename = outputDir + "/" + outputFilename + ".root";
  iputFilename = outputDir + "/" + inputFilename + ".root";
  ...
  // Load an iteration as the pseudo-data and pseudo-true.
  // We'll also load the response from a file, so then we don't have to rerun it.
  std::unique_ptr<TFile> f = TFile::Open(outputFilename.c_str());
  auto h2PseudoData = f.Get("Bayesian_Foldediter" + std::to_string(nIterToLoad));
  auto h2PseudoTrue = f.Get("Bayesian_Unfoldediter" + std::to_string(nIterToLoad));
  auto response = f.Get("
